...
public class Harvesting extends BasicUpgrade {
	...
	private static void doBlock(IBetterChest chest, int tick, World world, ItemStack item,
		...
		else if (block != null && (block instanceof BlockMelon || block instanceof BlockPumpkin)) {
			ItemStack itemS = new ItemStack(block.getItemDropped(meta, world.rand, 0), block.quantityDropped(meta, 0, world.rand), block.damageDropped(meta));
			if (InvUtil.putIntoFirstSlot(chest, itemS, true) == null) {
				InvUtil.putIntoFirstSlot(chest, itemS, false);
				world.setBlockToAir(xcoord, ycoord, zcoord);
			}
		}
		else if (block != null && (block instanceof BlockCactus || block instanceof BlockReed)) {
			int i = 0;
