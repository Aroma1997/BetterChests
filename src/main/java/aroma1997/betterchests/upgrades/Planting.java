...
public class Planting extends BasicUpgrade {
	...
	public void updateChest(IBetterChest chest, int tick, World world,
		...
		for (int i = Reference.Conf.PLANTS_START; i <= Reference.Conf.PLANTS_START + Reference.Conf.PLANTS_HEIGHT; i++) {
			doBlock(chest, tick, world, item, xcoord, ycoord + i, zcoord, slot);
		}
	}
	
	private static void doBlock(IBetterChest chest, int tick, World world, ItemStack item, int xcoord, int ycoord, int zcoord, int slot) {
		if (slot == -1) return;
