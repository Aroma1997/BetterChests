					...
					for (int y = -3; y < 3; y++) {
						...
						if (block == Blocks.dirt || block == Blocks.grass) {
							...
							if (blockAbove == null || blockAbove == Blocks.air || blockAbove instanceof BlockTallGrass || blockAbove instanceof BlockFlower) {
								...
								if (itemPlant == null || itemPlant.stackSize <= 0) return;
								IPlantable pl = (IPlantable) itemPlant.getItem();
								if (Block.getBlockFromItem((Item)pl).canPlaceBlockAt(world, xcoord, ycoord + 1, zcoord)) {
									world.setBlock(xcoord, ycoord + 1, zcoord, pl.getPlant(world, xcoord, ycoord + 1, zcoord), pl.getPlantMetadata(world, xcoord, ycoord, zcoord), 3);
