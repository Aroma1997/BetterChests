...
public class TileEntityBChest extends TileEntity implements IBetterChest,
		ISpecialInventory, IAromaWrenchable {
	...
	public void validate() {
		...
		if (!worldObj.isRemote) {
			fplayer = FakePlayerFactory.get((WorldServer) worldObj,
					new GameProfile("", "Aroma1997BetterChests"));
			fplayer.posX = xCoord;
			fplayer.posY = yCoord;
			fplayer.posZ = zCoord;
		}
	...
	public boolean isUseableByPlayer(EntityPlayer par1EntityPlayer) {
		...
		if (MinecraftServer
				...
				|| player.equalsIgnoreCase(par1EntityPlayer
						.getCommandSenderName())) {
			return true;
		}
		return false;
	...
	private void doNormalChestUpdate() {
		...
		if (this.numUsingPlayers > 0 && this.lidAngle == 0.0F) {
			...
			this.worldObj.playSoundEffect(d1, this.yCoord + 0.5D, d2,
		...
		if (this.numUsingPlayers == 0 && this.lidAngle > 0.0F
			...
			if (this.lidAngle < f2 && f1 >= f2) {
				...
				this.worldObj.playSoundEffect(d2, this.yCoord + 0.5D,
						d0, "betterchests:chest.bchestclose", 0.5F,
						this.worldObj.rand.nextFloat() * 0.1F + 0.9F);
			}
