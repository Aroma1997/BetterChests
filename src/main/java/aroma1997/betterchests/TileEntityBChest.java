...
public class TileEntityBChest extends TileEntity implements IBetterChest, ISpecialInventory,
	...
	public Packet getDescriptionPacket() {
		...
		writeToNBT(nbt);
		S35PacketUpdateTileEntity packet = new S35PacketUpdateTileEntity(xCoord, yCoord, zCoord,
		        worldObj.provider.dimensionId, nbt);
		return packet;
	}
	...
	private void doNormalChestUpdate() {
		...
		if (numUsingPlayers > 0 && lidAngle == 0.0F) {
			...
			worldObj.playSoundEffect(d1, yCoord + 0.5D, d2, "betterchests:chest.bchestopen", 0.5F,
		...
		if (numUsingPlayers == 0 && lidAngle > 0.0F || numUsingPlayers > 0 && lidAngle < 1.0F) {
			...
			if (lidAngle < f2 && f1 >= f2) {
				...
				worldObj.playSoundEffect(d2, yCoord + 0.5D, d0, "betterchests:chest.bchestclose",
	...
	public void setAmountUpgrade(ItemStack upgrade, int amount) {
		for (ItemStack item : upgrades) {
			if (ItemUtil.areItemsSameMatching(item, upgrade, ItemMatchCriteria.ID,
			        ItemMatchCriteria.DAMAGE, ItemMatchCriteria.NBT)) {
				if (amount <= 0) {
